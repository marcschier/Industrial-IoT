...
namespace Microsoft.Azure.IIoT.Storage.CouchDb.Clients {
    ...
    internal sealed class CouchDbCollection : IItemContainer {
        ...
        public Task<IDocumentInfo<T>> UpsertAsync<T>(T newItem,
            ...
            try {
                var newDoc = new CouchDbDocument<T>(newItem, _client.Mapper, id);
                if (!string.IsNullOrEmpty(etag)) {
                    ...
                    try {
                        ...
                        if (doc == null) {
                            // Add new
                            _client.GetCollection(Name).Insert(newDoc.Bson);
                        }
                            ...
                            // Update existing
                            _client.GetCollection(Name).Update(newDoc.Bson);
                        }
                        _client.Commit();
                    }
                    catch {
                    ...
                    // Plain old upsert
                    _client.GetCollection(Name).Upsert(newDoc.Bson);
                }
                _client.Checkpoint();
        ...
        public Task<IDocumentInfo<T>> ReplaceAsync<T>(IDocumentInfo<T> existing,
            ...
            try {
                var newDoc = new CouchDbDocument<T>(newItem, _client.Mapper, existing.Id);
                _client.BeginTrans();
                try {
                    // Get etag and compare with this here
                    var doc = FindAsync<T>(existing.Id);
                    if (doc == null) {
            ...
            try {
                if (!string.IsNullOrEmpty(etag)) {
                    ...
                    try {
                        ...
                        _client.GetCollection(Name).Delete(id);
                        _client.Commit();
                    }
                    catch {
                        _client.Rollback();
        ...
        public IQuery<T> CreateQuery<T>(int? pageSize, OperationOptions options) {
            return new ServerSideQuery<T>(this, _client.GetCollection<T>(Name).Query(), pageSize);
        }
        ...
        internal sealed class JsonDocument : CouchDocument {

            /// <summary>
            /// Actual document
            /// </summary>
            [JsonExtensionData]
            public IDictionary<string, JToken> Document;
        }
            ...
            public Task<IEnumerable<IDocumentInfo<T>>> ReadAsync(CancellationToken ct) {
                lock (_lock) {
                    var result = _items.Count != 0 ? _items.Dequeue()
                        : Enumerable.Empty<CouchDbDocument<T>>();
                    if (result == null) {
                        _collection._queryStore.Remove(_continuationToken);
                    }
        ...
        internal sealed class ClientSideQuery<T> : IQuery<T> {
            ...
            internal ClientSideQuery(CouchDbCollection collection, IQueryable<T> queryable,
                int? pageSize) {
            ...
            public IResultFeed<IDocumentInfo<T>> GetResults() {
                var results = _queryable.AsEnumerable()
                    .Select(d => new CouchDbDocument<T>(d, _collection._client.Mapper));
