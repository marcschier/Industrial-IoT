...
namespace Microsoft.IIoT.Azure.IoTEdge.Clients {
    ...
    public class IoTEdgeEventClientTests : IClassFixture<IoTEdgeEventClientFixture> {
        ...
        public async Task SendDeviceEventTest1Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                await queue.SendEventAsync(resource, data, contentType, "Test1", contentEncoding).ConfigureAwait(false);
        ...
        public async Task SendDeviceEventTest2Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "TEst1", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "TeST1", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "TeSt1", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "TesT1", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, data, contentType, "test1", contentEncoding).ConfigureAwait(false);
        ...
        public async Task SendDeviceEventTestBatch1Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                await queue.SendEventAsync(resource,
        ...
        public async Task SendDeviceEventWithCallbackTest1Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                queue.SendEvent(resource, data, contentType,
        ...
        public async Task SendDeviceEventWithCallbackTest3Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                var expected = 4;
                var actual = new TaskCompletionSource<int?>(TaskCreationOptions.RunContinuationsAsynchronously);
                queue.SendEvent(resource, data, fix.Create<string>(),
        ...
        public async Task SendDeviceEventWithCallbackLargeNumberOfEventsAsync(int max) {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                Enumerable.Range(0, max)
                    .ToList()
                    .ForEach(i => queue.SendEvent(resource, data, contentType,
        ...
        public async Task SendModuleEventTest1Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                await queue.SendEventAsync(resource, data, contentType, "test2", contentEncoding).ConfigureAwait(false);
        ...
        public async Task SendModuleEventTest2Async() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "Test2", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "Test2", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "Test2", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, data, contentType, "test2", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "Test2", contentEncoding).ConfigureAwait(false);
                await queue.SendEventAsync(resource, fix.CreateMany<byte>().ToArray(), contentType, "aaaaa", contentEncoding).ConfigureAwait(false);
        ...
        public async Task BadArgumentTestsAsync() {
            ...
            using (var harness = _fixture.GetHarness(resource)) {
                ...
                await Assert.ThrowsAsync<ArgumentNullException>(
                    () => queue.SendEventAsync(resource, (byte[])null, null, null, null)).ConfigureAwait(false);
                await Assert.ThrowsAsync<ArgumentNullException>(
                    () => queue.SendEventAsync(resource, (IEnumerable<byte[]>)null, null, null, null)).ConfigureAwait(false);
                await Assert.ThrowsAsync<ArgumentNullException>(
                ...
                Assert.Throws<ArgumentNullException>(
                    () => queue.SendEvent(resource, null, null, null, null, "test", (t, e) => { }));
                Assert.Throws<ArgumentNullException>(
                    () => queue.SendEvent<string>(resource, new byte[4], null, null, null, null, (t, e) => { }));
                Assert.Throws<ArgumentNullException>(
                    () => queue.SendEvent(resource, new byte[4], null, null, null, "test", null));
            }
        }
    }
}
