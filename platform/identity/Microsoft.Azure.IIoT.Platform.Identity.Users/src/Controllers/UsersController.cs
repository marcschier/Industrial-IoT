...
namespace Microsoft.Azure.IIoT.Platform.Identity.Users {
    using Microsoft.Azure.IIoT.Platform.Identity.Users.Filters;
    using Microsoft.Azure.IIoT.Platform.Identity.Users.Models;
    using Microsoft.Azure.IIoT.Platform.Identity.Users.Auth;
    using Microsoft.Azure.IIoT.Platform.Identity.Api.Models;
    ...
    public class UsersController : Controller {
        ...
        public async Task CreateUserAsync(
            ...
            var result = await _manager.CreateAsync(user.ToServiceModel()).ConfigureAwait(false);
            result.Validate();
        }
        ...
        public async Task<UserApiModel> GetUserByIdAsync(string userId) {
            ...
            var user = await _manager.FindByIdAsync(userId).ConfigureAwait(false);
            return user.ToApiModel();
        }
        ...
        public async Task DeleteUserAsync(string userId) {
            ...
            var user = await _manager.FindByIdAsync(userId).ConfigureAwait(false);
            var result = await _manager.DeleteAsync(user).ConfigureAwait(false);
            result.Validate();
        }
        ...
        public async Task AddClaimAsync(string userId,
            ...
            var user = await _manager.FindByIdAsync(userId).ConfigureAwait(false);
            var result = await _manager.AddClaimAsync(user, model.ToClaim()).ConfigureAwait(false);
            result.Validate();
        }
        ...
        public async Task RemoveClaimAsync(string userId, string type, string value) {
            ...
            var user = await _manager.FindByIdAsync(userId).ConfigureAwait(false);
            var result = await _manager.RemoveClaimAsync(user, new Claim(type, value)).ConfigureAwait(false);
            result.Validate();
        }
        ...
        public async Task AddRoleToUserAsync(string userId, string role) {
            ...
            var user = await _manager.FindByIdAsync(userId).ConfigureAwait(false);
            var result = await _manager.AddToRoleAsync(user, role).ConfigureAwait(false);
            result.Validate();
        }
        ...
        public async Task<IEnumerable<UserApiModel>> GetUsersInRoleAsync(
            string role) {
            var result = await _manager.GetUsersInRoleAsync(role).ConfigureAwait(false);
        ...
        public async Task RemoveRoleFromUserAsync(string userId, string role) {
            ...
            var user = await _manager.FindByIdAsync(userId).ConfigureAwait(false);
            var result = await _manager.RemoveFromRoleAsync(user, role).ConfigureAwait(false);
            result.Validate();
        }
