    ...
    public static class WriterGroupRegistrationEx {
        ...
        public static DeviceTwinModel Patch(this WriterGroupRegistration existing,
            ...
            if (update != null) {
                ...
                if (update?.WriterGroupId != null &&
                    ...
                    tags.Add(nameof(WriterGroupRegistration.WriterGroupId), update.WriterGroupId);
                }

                if (update?.SiteId != existing?.SiteId) {
                    tags.Add(nameof(WriterGroupRegistration.SiteId), update?.SiteId);
                }
                ...
                if (update?.Schema != existing?.Schema) {
                    properties.Add(nameof(WriterGroupRegistration.Schema),
                        update?.Schema == null ?
                            null : serializer.FromObject(update.Schema.ToString()));
                }

                if (update?.Encoding != existing?.Encoding) {
                    properties.Add(nameof(WriterGroupRegistration.Encoding),
                ...
                if (update?.NetworkMessageContentMask != existing?.NetworkMessageContentMask) {
                    properties.Add(nameof(WriterGroupRegistration.NetworkMessageContentMask),
                ...
                if (update?.DataSetOrdering != existing?.DataSetOrdering) {
                    properties.Add(nameof(WriterGroupRegistration.DataSetOrdering),
                ...
                if (update?.HeaderLayoutUri != existing?.HeaderLayoutUri) {
                    properties.Add(nameof(WriterGroupRegistration.HeaderLayoutUri),
                        update?.HeaderLayoutUri);
                }

                if (update?.SamplingOffset != existing?.SamplingOffset) {
                    properties.Add(nameof(WriterGroupRegistration.SamplingOffset),
                        update?.SamplingOffset);
                }

                if (update?.BatchSize != existing?.BatchSize) {
                    properties.Add(nameof(WriterGroupRegistration.BatchSize),
                        update?.BatchSize);
                }

                if (update?.Priority != existing?.Priority) {
                    properties.Add(nameof(WriterGroupRegistration.Priority),
                        update?.Priority);
                }

                if (update?.KeepAliveTime != existing?.KeepAliveTime) {
                    properties.Add(nameof(WriterGroupRegistration.KeepAliveTime),
                        update?.KeepAliveTime);
                }

                if (update?.PublishingInterval != existing?.PublishingInterval) {
                    properties.Add(nameof(WriterGroupRegistration.PublishingInterval),
                        update?.PublishingInterval);
                }

                if (update?.GroupVersion != existing?.GroupVersion) {
                    properties.Add(nameof(WriterGroupRegistration.GroupVersion),
                        update?.GroupVersion);
                }

                if (update?.MaxNetworkMessageSize != existing?.MaxNetworkMessageSize) {
                    properties.Add(nameof(WriterGroupRegistration.MaxNetworkMessageSize),
                ...
                if (!(offsetsEqual ?? true)) {
                    properties.Add(nameof(WriterGroupRegistration.PublishingOffset),
                ...
                if (!(localesEqual ?? true)) {
                    properties.Add(nameof(WriterGroupRegistration.LocaleIds),
            ...
            return new DeviceTwinModel {
                Id = (existing?.DeviceId ?? update?.DeviceId)
                    ?? PublisherRegistryEx.ToDeviceId(update?.WriterGroupId),
                Etag = existing?.Etag,
                Status = update == null || update.IsDisabled == null ? null :
        ...
        public static WriterGroupRegistration ToWriterGroupRegistration(this DeviceTwinModel twin,
            ...
            var registration = new WriterGroupRegistration {
                ...
                // Tags
                IsDisabled =
                    twin.Tags.GetValueOrDefault(nameof(WriterGroupRegistration.IsDisabled), twin.IsDisabled()),
                NotSeenSince =
                    twin.Tags.GetValueOrDefault<DateTime>(nameof(WriterGroupRegistration.NotSeenSince), null),
                WriterGroupId =
                    twin.Tags.GetValueOrDefault<string>(nameof(WriterGroupRegistration.WriterGroupId), null),
                SiteId =
                    twin.Tags.GetValueOrDefault<string>(nameof(WriterGroupRegistration.SiteId), null),
