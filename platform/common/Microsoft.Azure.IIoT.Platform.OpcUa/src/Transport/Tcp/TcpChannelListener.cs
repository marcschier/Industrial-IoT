    ...
    public sealed class TcpChannelListener : ITcpChannelListener, IStartable, IDisposable {
        ...
        private Socket BindSocket(IPAddress address, int port) {
            try {
                ...
                _logger.LogWarning(ex, "failed to create listening socket.");
                return null;
            }
        }
            ...
            while (true) {
                ...
                if (e.AcceptSocket != null && e.SocketError == SocketError.Success) {
                    try {
                        ...
                        _logger.LogDebug("Started channel {channelId} on {socket}...",
                ...
                if (error != SocketError.OperationAborted && token is Socket listeningSocket) {
                    ...
                    try {
                        ...
                        _logger.LogError(ex, "Unexpected error listening for a connections.");
                        // Stop listening
                    }
                }
                break;
