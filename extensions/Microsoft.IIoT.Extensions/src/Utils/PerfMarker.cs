...
namespace Microsoft.IIoT.Utils {
    ...
    public sealed class PerfMarker : IDisposable {
        ...
        public PerfMarker(ILogger logger, string name) {
            ...
            _sw = new Stopwatch();
            _sw.Start();
            _logger.LogTrace("Start {name} ...", _name);
        }
        ...
        public void StepCompleted(string step) {
            _logger.LogInformation("{name}: {step} took {elapsed}", _name, step, _sw.Elapsed);
            _sw.Restart();
        }
        ...
        public void Dispose() {
            _sw.Stop();
            _logger.LogInformation("{name} took {elapsed}.", _name, _sw.Elapsed);
        }
